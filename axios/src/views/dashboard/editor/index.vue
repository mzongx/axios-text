<template>
	<div class="dashboard-editor-container">
		<div class=" clearfix">
			<UserLogo :image="avatar">
				你的权限:<span class="pan-info-roles" v-for="item in roles">{{item}}</span>
			</UserLogo>
						...
						<div class="recent-articles-wrapper">
							<template v-if="recentArticles.length!=0">
								<div class="recent-articles-item" v-for="item in recentArticles">
									<span class="recent-articles-status">{{ item.status | articleStatus }}</span>
...
<script>
	...
	export default {
		...
		components: { UserLogo, countTo, MonthKpi, ArticlesChart },
		data() {
			return {
				statisticsData: {
					article_count: 1024,
